...
-- ╔════════════════════════════════════════╗
-- ║  COSMIC REDZ HUB  •  Rayfield Edition  ║
-- ╚════════════════════════════════════════╝
local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()

local _RayfieldWindow = Rayfield:CreateWindow({
    Name             = '✦ COSMIC REDZ HUB ✦',
    LoadingTitle     = 'COSMIC REDZ HUB',
    LoadingSubtitle  = 'discord.gg/SexKW4Fq5Z  •  COSMIC v2.1',
    ConfigurationSaving = {
        Enabled    = true,
        FolderName = 'Redz | redz lib v5',
        FileName   = 'CosmicRedzHub',
    },
    KeySystem = false,
})

-- RedzLib-compatible shim so every AddToggle/AddButton/etc below works unchanged
local u616 = {}
u616._win = _RayfieldWindow

function u616.MakeTab(_, args)
    local _tab = _RayfieldWindow:CreateTab(args[1] or 'Tab', args[2] or 4483362458)
    local _obj = {}

    function _obj:AddSection(a)
        local _t = type(a) == 'table' and (a[1] or a.Title or a.Name or '') or tostring(a)
        _tab:CreateSection(_t)
    end

    function _obj:AddToggle(a)
        _tab:CreateToggle({
            Name         = a.Name or a.Title or '',
            Description  = a.Description or '',
            CurrentValue = a.Default or false,
            Flag         = a.Flag or (a.Name or a.Title or tostring(math.random(1,999999))),
            Callback     = a.Callback or function() end,
        })
    end

    function _obj:AddButton(a)
        _tab:CreateButton({
            Name        = a.Title or a.Name or '',
            Description = a.Description or '',
            Callback    = a.Callback or function() end,
        })
    end

    function _obj:AddDropdown(a)
        _tab:CreateDropdown({
            Name          = a.Name or a.Title or '',
            Description   = a.Description or '',
            Options       = a.Options or {},
            CurrentOption = a.Default and {a.Default} or {},
            Flag          = a.Flag or (a.Name or a.Title or tostring(math.random(1,999999))),
            Multi         = false,
            Callback      = a.Callback or function() end,
        })
    ...
    function _obj:AddSlider(a)
        _tab:CreateSlider({
            Name          = a.Title or a.Name or '',
            Description   = a.Description or '',
            Range         = {a.Min or 0, a.Max or 100},
            Increment     = a.Increment or 1,
            CurrentValue  = a.Default or a.Min or 0,
            Flag          = a.Flag or (a.Title or a.Name or tostring(math.random(1,999999))),
            Callback      = a.Callback or function() end,
        })
    end

    function _obj:AddParagraph(a)
        _tab:CreateParagraph({
            Title   = a.Title or '',
...
function u616:Minimize(state)
    -- Rayfield handles its own minimize; no-op shim keeps calls safe
end
local _WmGui = Instance.new('ScreenGui')
_WmGui.Name = 'CosmicWatermark'
_WmGui.ResetOnSpawn = false
_WmGui.DisplayOrder = 9998
_WmGui.Parent = game.CoreGui

local _WmFrame = Instance.new('Frame')
_WmFrame.Size = UDim2.new(0, 260, 0, 22)
_WmFrame.Position = UDim2.new(0.5, -130, 0, 4)
_WmFrame.BackgroundColor3 = Color3.fromRGB(8, 0, 18)
_WmFrame.BackgroundTransparency = 0.08
_WmFrame.BorderSizePixel = 0
_WmFrame.Parent = _WmGui

local _WmCorner = Instance.new('UICorner')
_WmCorner.CornerRadius = UDim.new(0, 12)
_WmCorner.Parent = _WmFrame

local _WmGrad = Instance.new('UIGradient')
_WmGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0,   Color3.fromRGB(18, 0, 40)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(6,  0, 16)),
    ColorSequenceKeypoint.new(1,   Color3.fromRGB(14, 0, 32)),
})
_WmGrad.Rotation = 90
_WmGrad.Parent = _WmFrame

local _WmStroke = Instance.new('UIStroke')
_WmStroke.Color = Color3.fromRGB(160, 0, 230)
_WmStroke.Thickness = 1.5
_WmStroke.Parent = _WmFrame

-- Watermark stroke pulse
task.spawn(function()
    local ts3 = game:GetService('TweenService')
    while _WmFrame and _WmFrame.Parent do
        ts3:Create(_WmStroke, TweenInfo.new(1.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Color = Color3.fromRGB(215, 90, 255),
            Thickness = 2.2
        }):Play()
        task.wait(1.8)
        ts3:Create(_WmStroke, TweenInfo.new(1.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            Color = Color3.fromRGB(100, 0, 160),
            Thickness = 1.2
        }):Play()
        task.wait(1.8)
    end
...
local _WmLabel = Instance.new('TextLabel')
_WmLabel.Size = UDim2.new(1, 0, 1, 0)
_WmLabel.BackgroundTransparency = 1
_WmLabel.Text = '✦ COSMIC REDZ HUB  •  discord.gg/SexKW4Fq5Z ✦'
_WmLabel.TextColor3 = Color3.fromRGB(190, 100, 255)
_WmLabel.Font = Enum.Font.GothamBold
_WmLabel.TextSize = 10
_WmLabel.TextXAlignment = Enum.TextXAlignment.Center
_WmLabel.TextStrokeTransparency = 0.6
_WmLabel.TextStrokeColor3 = Color3.fromRGB(80, 0, 130)
_WmLabel.Parent = _WmFrame

-- Watermark label shimmer
task.spawn(function()
    local ts4 = game:GetService('TweenService')
    while _WmLabel and _WmLabel.Parent do
        ts4:Create(_WmLabel, TweenInfo.new(2.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            TextColor3 = Color3.fromRGB(230, 150, 255)
        }):Play()
        task.wait(2.2)
        ts4:Create(_WmLabel, TweenInfo.new(2.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            TextColor3 = Color3.fromRGB(150, 40, 220)
        }):Play()
        task.wait(2.2)
    end
end)

-- Rotating gradient on watermark
task.spawn(function()
    local r = 90
    while _WmGrad and _WmGrad.Parent do
        r = (r + 0.3) % 360
        _WmGrad.Rotation = r
        task.wait(0.04)
    end
...
pcall(function() loadstring(game:HttpGet('https://raw.githubusercontent.com/AnhDangNhoEm/TuanAnhIOS/refs/heads/main/koby'))() end)
v654:AddToggle({
    Name = 'Bring Mod',
